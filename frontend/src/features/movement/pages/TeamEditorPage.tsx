  ...
  return (
    <Card className="surface-card">
      <Typography.Title level={3} className="section-title">
        {isEditing ? 'Create/Edit Team' : 'Create Team'}
      </Typography.Title>
      <Form
        form={form}
        layout="vertical"
        onFinish={(values) => {
          const duplicate = teams.some((item) => item.id === values.id && item.id !== team?.id)
          ...
          modal.confirm({
            ...
            content: 'Team mới sẽ được khởi tạo toàn bộ station ở trạng thái New.',
            okText: 'Xác nhận',
            cancelText: 'Hủy',
            onOk: () => {
              saveTeam(values, team?.id)
              message.success(isEditing ? 'Đã cập nhật team' : 'Đã tạo team mới')
      ...
      >
        ...
        <Form.Item label="totalTime (min)" name="totalTimeMinutes" rules={[{ required: true }]}>
          <InputNumber min={0} className="full-width" />
        </Form.Item>
